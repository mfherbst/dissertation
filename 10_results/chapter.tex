...
Even though the convergence rate drops a little until 4 shells have been reached,
the tail end of the graphs still indicates a linear convergence
for all example calculations shown.
This result is certainly rather nice,
but in the light of the completeness property of the Coulomb-Strumians
not really surprising either.

The question is now if and how we could improve this convergence rate.
Let us consider the number of basis functions $\Nbas$
in a \CS basis including all shells up to $\nmax$.
We find
\begin{align}
	\nonumber
