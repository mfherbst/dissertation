...
This section will introduce and contrast various classes of functions,
which can be used to build a basis set $\{\varphi_\mu\}_{\mu\in\Ibas}$
for formulating and solving the discretised \HF problem
as it was introduced in section \vref{sec:DiscreteHF}.
For this we will first introduce a set of desirable properties
for a basis,
not only in the context of \HF but in quantum-chemistry as a whole.
Next we will proceed to discuss four types of basis functions in depth.
The historic Slater-type orbitals~({\STO}s),
the most commonly employed contracted Gaussian-type orbitals~({\cGTO}s),
Coulomb-Strumians as a representative of Sturmian-type orbitals
as well as a fully numerical approach based on Finite Elements
as an example for numerical basis functions.




We will both introduce desirable properties
and measures for judging the quality of 


Our discussion will involve both the usual approach
of so-called contracted Gaussian-type atomic orbitals
as well as alternat


discussing common basis fu

We will discuss a set of desirable properties
for a good basis set for \HF or in fact any electronic structure theory method.
Based on these criteria we will contrast several types of basis functions,
which are either used very much in practice
for atomic and molecular calculations or which show some very noteworthy properties.

These are typically not extremely suitable in the single atom or single molecule setting,
which we focus on,

 
Even though this selection represents a fair amount of methods
used in electronic structure theory nowadays,
...
Plane waves neglected
because not dealing with extended systems or condensed matter
