...
At least they should do so within a provided error bound.
In other words one needs to make sure that
$\mat{C}^{(\infty)} \in \mathcal{C}$ and
that the Pulay error with the unmodified matrix $\matFnfull$
\eqref{eqn:PulayError} is below a threshold.
