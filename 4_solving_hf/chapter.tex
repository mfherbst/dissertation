...
Discussion done in formalism of \UHF,
since \RHF works the same, but less blocks.
...
% Guess methods
% algorithm
% convergence check (Pulay error \eqref{eqn:PulayError})
It gives an excellent measure for the convergence of such an \SCF procedure.
Typically one consideres an \SCF to be converged if
\[ \norm{ \mat{e}^{(n)} }_\text{frob} \leq  \epsilonconv \]


A few \SCF properties~\cite{Cances2000}
...
\section{General remark about basis functions}
Why can we not use everything?
