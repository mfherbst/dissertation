...
In the previous sections we mentioned a few typical approaches
to solve the \HF minimisation problem in an \SCF sense.
Needless to say that not all algorithms are suitable
to each use case.
For this reason in practice often a mixture of some of these methods
is employed in order to guarantee fast and reliable convergence.
This section represents my own judgement of the situation
and is entirely based on my hands-on experience and not at all
on proper scientific evaluation or comparison%
...
It should therefore not be taken as a final answer,
rather a guideline.

In the beginning of the procedure \ODA or \tODA are very useful,
essentially directing the coefficients reliably into the right direction.
The energy \DIIS can be seen as an accelerated improvement of those methods,
which is certainly recommendable for \cGTO-based discretisation
as the initial \SCF algorithm in my point of view.

In the intermediate steps a Pulay \DIIS shows typically a faster
...
making it easier to explore directions,
which to lead to nearby stationary points.

In the later steps the \DIIS becomes numerically more unstable,
but since we are close to the \SCF minimum,
second-order \SCF schemes like QCSCF now show the fastest convergence
and should be used to obtain a highly-accurate \SCF minimum
after the \DIIS becomes less effective.
