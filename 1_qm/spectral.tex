...
\todoil{Check that closed is not needed for Fiedrichs extension
and if not drop it}

Before diving into the definitions and theorems regarding
the spectra of operators, we introduce the term \newterm{closed operator}.

\begin{defn}
	A linear operator $\Op{A}$ is closed if its graph
	\[ \text{Gr}(\Op{A}) := \{ (u, \Op{A}u) \, | \, u \in D(\Op{A}) \} \]
	is a closed subspace of $\hilbert \times \hilbert$.
\end{defn}
This definition is rather technical.
We will only need the result that all self-adjoint operators are closed.
...
\begin{prop}
	...
	and
	\[ \spacespan \{ u_k \}_{k \in \mathcal{I}} = \hilbert. \]
	\begin{proof}
