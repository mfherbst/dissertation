...
As expected one may easily show, that~\todo{Find citation or do proof.}
\begin{prop}
...
and the Laplace operator in $3$, expressed in spherical polar coordinates
are related by
...
the Hydrogenic Schrödinger operator \eqref{eqn:OpHydrogen}
\[
...
One can show~\cite{Davies2007} that $\sigma_C(\Op{H}) = [0, \infty)$
and all discrete eigenvalues from $\sigma_P(\Op{H})$ are less than zero.
In other words the discrete spectrum of $\Op{H}$ can be conveniently
...
\todoil{Mention that radial part is complete in the radial sense due to Sturm-Liouville theory.}

If one walks through the derivation properly,
one notices that as a consequence of the Sturm-Liouville form of the involved equations,
the principle quantum number $n$ is always larger than zero
...
We will now argue that the solutions $\Psi \in H^2(\R^3, \C)$.
...
Use the results from example \vref{exmp:H1sH2}
to show that the solutions $\Psi \in H^2(\R^3, \C)$.

% ----------------------------------------------

% Or move this to take-away
