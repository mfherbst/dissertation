...
In infinite dimensions, where not every injective operator can be made
surjective by a trivial operator extension,
one can therefore find points in the spectrum, which are not eigenvalues
in the classical sense.
This is expressed in the next definition.
...
\end{defn}
\todoil{Rephrase?}
The so-called RAGE\footnote{After the initials of the discoverers 
Ruelle, Amrein, Georgescu and Volker Enss}
theorem
\todo{Lookup references}
allows for a physical interpretation of the point spectrum $\sigma_P(\Op{A})$
and the continuous spectrum $\sigma_C(\Op{A})$.
It states that the eigenfunctions associated with the eigenvalues in $\sigma_P(\Op{A})$
correspond to the so-called
\newterm{bound state}s of an operator,
\ie those states which have --- at all times --- a non-vanishing function value
only in a finite region of space.
On the other hand $\sigma_P(\Op{A})$ can be associated with the
\newterm{scattering state}s,
which will eventually vanish from any arbitrarily large, bounded part of space.
...
that it is only possible to construct reliable numerical methods
for obtaining approximate eigenvalues
for the discrete spectrum of an operator.

Before we try to rationalise this observation,
let us discuss the special case of compact, self-adjoint operators.
...
may at most contain the value $0$.
If the operator is self-adjoint as well, we can state as follows.
\begin{prop}
...
\todoil{Rephrase}
Compact operators show the properties we would hope for in quantum mechanics.
We can always express an arbitrary state in terms of the Hilbert space
in the eigenstates of an operator.
Furtheremore they are numerically easy to deal with.
Unfortunately most operators of \QM are not compact,
since they are not even bounded.
This includes the Laplacian and the Schrödinger
operators as we shall see.

On the other hand if the Hilbert space is finite-dimensional
all operators are rather nice.
\begin{prop}
	\label{prop:FiniteDimCompact}
	Any linear operator on a finite-dimensional Hilbert space $\hilbert$ is compact.
	\todoil{Find reference}
\end{prop}

\begin{rem}
Proposition \ref{prop:FiniteDimCompact} has a lot of noteworthy consequences:
\begin{itemize}
	\item $\Op{A}$ may be viewed as a multiplication by a finite matrix.
...
% Issues with the essential spectrum
% 
