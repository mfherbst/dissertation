...
\subsection{Hilbert spaces of quantum mechanics}
...
at each point in space $(x_1, \ldots x_N)$.
...
\begin{prop}
	...
	\end{equation}
	\todoil{Fabian:\\
		das innere product is fuer allgemeine functionen nicht wohldefiniert.
		ich wuerde vorschlagen, schreib ``wherever well defined''
		dazu oder erwaehne lebesgue messbarkeit.
		Weiter wuerde ich die ``norm'' als abbildung definieren,
		da es ja lediglich eine semi-norm ist. Dann kann man $L^2$
		definieren mit der bemerkung, dass eine aquivalenz klasse eingefuehrt
		wird bzgl. der die ``norm-abbildung'' $\norm{\slot}$ eine norm ist.
	}
...
\noindent
Luckily one can show~\cite{Werner2011}
\begin{prop}
...
\begin{exmp}
	...
	\todoil{
		Loch im Beweis (Fabian):\\
-> mhhh..., ich bin mir nicht sicher ob das durchgeht. nur weil eine function nirgends gegebn unendlich strebt heisst nicht, dass sie im $L^2$ sinn beschraenkt ist. das gilt nur fuer functionen auf einer menge von endlichem mass, was $\mathbb{R}^3$ nicht ist. ich wuerde folgendes vorschlagen: $\Psi$ ist stetig also lebesgue messbar, damit ist die $L^2$ norm wohldefiniert. weiter fallen die gaussfunctionen 'schnell genug' ab (sind im schwarzraum) [referenz... wahrscheinlich auch im werner], s.d. die norm insbesondere endlich ist. dann kann man den beweis abschliessen, wie du es gemacht hast: [Due to ....].
	}
	\begin{proof}
