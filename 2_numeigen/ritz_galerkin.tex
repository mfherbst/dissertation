...
What we can achieve, however, is a method to obtain
...
we can assume that our operators
always possess a discrete spectrum, which is located below the essential spectrum.
Furthermore those bound states, which are of interest to us,
are always located at the lower end of the discrete spectrum.
Using the aforementioned results we can
therefore sketch an approximation method for our setting.
...
\begin{rem}[Approximation of the bottom of the discrete spectrum]
	...
	by selecting larger and larger sets $\{\chi_\mu\}_{\mu \in \Ibas}$
	of $\Nbas = \abs{\Ibas}$ orthonormal basis functions $\chi_\mu \in U$
	as the bases.
	...
	$\spacespan \{\chi_\mu\}_{\mu \in \Ibas}$ will tend towards $U$.
	Thus $\spacespan \{\chi_\mu\}_{\mu \in \Ibas}$ eventually allows
	...
	basis sets $\{\chi_\mu\}_{\mu \in \Ibas} \subset U$.
...
\begin{rem}[Discrete formulation of \eqref{eqn:varProblem}]
	...
	\noindent
	If $S_{\Nbas} = \spacespan \{\chi_\mu\}_{\mu \in \Ibas}$, we can expand
	\[ v^{(n)}_i = \sum_{\nu\in\Ibas} C^{(n)}_{\nu i} \chi_\nu \qquad \text{with} \quad C^{(n)}_{\nu i} \equiv \braket{\chi_\nu}{v^{(n)}_i} \]
	...
	\begin{equation}
		...
		\begin{aligned}
			...
			\forall \chi_\mu \in S_{\Nbas}:
			\quad \sum_{\nu\in\Ibas} C^{(n)}_{\nu i} \, a(\chi_\mu, \chi_\nu)
			&= \lambda_i^{(n)} \sum_{\nu\in\Ibas} C^{(n)}_{\nu i} \, \braket{\chi_\mu}{\chi_\nu}\\
			1 &= 
			\sum_{\nu\in\Ibas} \sum_{\mu\in\Ibas} \cc{\left(C^{(n)}_{\mu i}\right)} \braket{\chi_\mu}{\chi_\nu} C^{(n)}_{\nu i}
		\end{aligned}
		\right\}.
	\end{equation}
	...
	with elements
	\begin{align*}
		A_{\mu\nu} &= a(\chi_\mu, \chi_\nu) & \big(c^{(n)}_i\big)_\mu &= \braket{\chi_\nu}{v^{(n)}_i} = C^{(n)}_{\mu i}
	\end{align*}
...
let us summarise the requirements towards the basis functions $\chi_\mu$
...
\begin{rem}
In remark \vref{rem:NumericalEssentialSpectrum}
we claimed that it was very difficult
to obtain numerical approximations to the essential spectrum.
Because of the min-max theorem \vref{thm:CourantFischer},
we now have some theoretical justification for this.
Corollary \vref{cor:Convergence} namely
gives us,
that all eigenvalues from a Ritz-Galerkin approximation
of $\Op{A}$ tend to $\lambda_i(\Op{A})$ 
as we increase the subspace size.
...
Another consequence of this behaviour is,
...
As soon as the eigenvalues $\lambda_i^{(n)}$
of $\mat{A}^{(n)}$ become large enough,
they merely an artificial discretisation of the essential spectrum,
which will all tend to $\inf \sigma_\text{ess}(\Op{A})$
as the basis set is increased.
Since the convergence in corollary \vref{cor:Convergence} is always
from above it is not always easy to judge,
where the line between trustworthy and not trustworthy eigenpairs of $\mat{A}^{(n)}$
should be drawn.
In either case the bottom end of the spectrum of $\mat{A}$
will carry physical significance if the basis
