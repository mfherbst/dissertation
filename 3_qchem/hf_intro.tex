...
\nomenclature{$\rho_\Theta(\vec{r})$}{
	Electron density, see equation \eqref{eqn:HFDensityNoSpin}.}
\nomenclature{$\gamma_\Theta(\vec{r}_1,\vec{r}_2)$}{
	One-particle reduced density matrix, see equation \eqref{eqn:HFDensityNoSpin}.}
...
\begin{rem}[Invariance under orbital rotations]
	...
	\todoil{Give some physical insight into this:\\
		...
		Considering the TISE, does this just mean
		that the state is invariant over time?
		(leading to a consistent description\ldots)
	}
\end{rem}
...
A similar mathematical statement making
...
From the unfilled shell property (g) $\varepsilon_{\Nelec} < \varepsilon_{\Nelec+1}$i%
~\cite{Bach1994},
