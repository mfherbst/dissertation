...
\nomenclature{$\NelecA, \NelecB$}{Number of $\alpha$ or $\beta$ electrons,
	respectively.
...
\nomenclature{$\rho_\Theta(\vec{r})$}{Electron density.}
...
\begin{rem}[Spin-adapted formulation of \HF]
	...
	% TODO OPTIONAL \to doil{Give an example}
...
\begin{rem}[Invariance under orbital rotations]
	...
	determinant $\Phi_0$.
	\todoil{Give some physical insight into this:\\
		Unitary transformations usually describe a time-evolution.
		Considering the TDSE, this means that
		the HF ground state (not interacting with an external perturbation by
		correlation or an external field) is invariant over time.
	}
\end{rem}
...
% TODO OPTIONAL Skipped for time reasons
...
I am unaware of a similar mathematical statement making
a guarantee that the \HF procedure
gives a sensible ground state for negatively charged systems.
...
not even for the resulting ground state density.
...
\nomenclature{$\mat{C}^\alpha$}{Matrix of occupied spin-up molecular orbital
	coefficients of size \linebreak $\Nbas \times \NelecA$.}
