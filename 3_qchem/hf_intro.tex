...
Theorem \ref{thm:MathematicalHF} provdides the mathematical
justification for the \HF procedure as it is performed
in almost every quantum-chemistry program these days.
We will discuss the relevance of the stated results
in the following section
together with the discretised versions of the \HF ansatz.
...
\begin{rem}[Discretised \HF variational problem]
	...
	For this reason it is hardly ever performed in practice.
	...
	\begin{itemize}
		...
		\item the \newterm{nuclear attraction matrix} $\mat{V}_0$
			...
			\begin{equation}
				...
				\label{eqn:Vbas}
			\end{equation}
		\item the \newterm{Coulomb matrix} $\matJfull$ has
			identical $\alpha$ and $\beta$ blocks of elements
			\begin{equation}
...
\begin{rem}
	...
	with the $\alpha$-block describing the physics in the
	spin-up component
	and the $\beta$-block describing the spin-down component.
	Apart from the exchange matrix $\matK$
	and naturally the density matrix $\mat{D}$ as well as the coefficient matrix $\mat{C}$
	...
	\begin{equation}
		...
		\norm{
		\tp{\left(\mat{C}^\alpha\right)} \mat{S}^\alpha \mat{C}^\beta
		}_\text{frob}^2
	\end{equation}
	where
	\[ S^2_\text{exact}
	...
	\eqref{eqn:HFStiefel} and \eqref{eqn:HFGrassmann} are comparatively complex.
	The structure of the parameters in the form of the coefficient matrix
	$\mat{C}$ and the density matrix $\mat{D}$
	does not follow a standard Euclidean geometry.
	Much rather they lie on objects called manifolds.
	In fact $\mathcal{C}$ is a subset of a Stiefel manifold
	and $\mathcal{P}$ is a subset of a Grassmann manifold.
	This aspect becomes apparent at many places when designing rigorous
	algorithms for solving the \HF problem,
	where it often gives rise to various peculiarities
	and explains why the intuitive approach not always works.
\end{rem}
...
for the Fock operator at the minimal point.
Theorem \ref{thm:MathematicalHF} guarantees
that it satisfies all the spectral requirements for applying the Ritz-Galerkin ansatz
of remark \vref{rem:DiscreteFormulation},
namely a discrete spectrum below the essential spectrum.
Choosing thase same basis $\{\varphi_\mu\}_{\mu\in\Ibas}$
as in remark \ref{rem:HFDiscreteVariational}
and projecting \eqref{eqn:HFequations} problem onto this basis
yields the discretised \HF equations for $i=1,\ldots,\Nelec$
\begin{equation}
	\matFfull \vec{c}_i = \varepsilon_i \mat{S} \vec{c}_i,
	\label{eqn:HFDiscreteEquations}
\end{equation}
where the \newterm{Fock matrix} $\matFfull$ is the discretised version of $\Op{F}$
and we further require $\tp{\vec{c}}_i \vec{c}_j = \delta_{ij}$.
This equation can be solved for up $\Norb \leq \Nbas$ eigenpairs
...
\begin{equation}
	\mat{C}_F = \mm{ \vec{c}_1 & \vec{c}_2 & \cdots & \vec{c}_{\Norb}},
	\label{eqn:DefinitionCoefficient}
\end{equation}
which contains all eigenfunctions we solved \eqref{eqn:HFDiscreteEquations} for.

Applying the spin restrictions of unrestricted \HF
exactly as in remark \ref{rem:HFDiscreteVariational},
we can write the Fock matrix $\matFfull$ as
\begin{equation}
...
Notice that this matrix is Hermitian and block diagonal
\[ \matF = \mm{\mat{F}^\alpha & 0 \\ 0 & \mat{F}^\beta}, \]
such that the eigenproblem \eqref{eqn:HFDiscreteEquations}
can always be solved by solving in both blocks $\sigma \in \{\alpha, \beta\}$
\[
	\mat{F}^\sigma\left[\mat{C}\tp{\mat{C}}\right] \vec{c}_i^\sigma
	= \varepsilon_i \mat{S}^\alpha \vec{c}_i^\sigma
\]
separately,
for $\Norb/2$ (assume $\Norb$ is even) eigenpairs each.
...
Those being part of $\mat{C}$ by the Aufbau principle are called \newterm{occupied orbitals}
as they are in some sense occupied by electrons.
Consequently all other orbitals are called unoccupied or \newterm{virtual orbitals}.
...
the minimiser $\mat{C} \in \mathcal{C}$ of \vref{eqn:HFStiefel}
satisfies exactly the discretised \HF equations \eqref{eqn:HFDiscreteEquations}
such that
\[
	\mat{F}\left[\mat{C}\tp{\mat{C}}\right] \mat{C} = \mat{S} \mat{C} \mat{E}
\]
where
\[
	\mat{E} = \text{diag}(\varepsilon_1^{(n)}, \varepsilon_2^{(n)}, \ldots,
		\varepsilon_{\Nelec}^{(n)}).
\]
This condition can be equivalently expressed as~\cite{Pulay1982}
\begin{equation}
	\mat{F}\left[\mat{C}\tp{\mat{C}}\right]
	\mat{C}\tp{\mat{C}}
	\mat{S} - \mat{S} \mat{C}\tp{\mat{C}} \mat{F}\left[\mat{C}\tp{\mat{C}}\right]
	= \mat{0}
...
\[
	\mat{E}^{(n+1)}
	= \text{diag}(\varepsilon_1^{(n+1)},
	\varepsilon_2^{(n+1)}, \ldots,
	\varepsilon_{\Norb}^{(n+1)}).
\]
such that
\begin{equation}
	\matFnfull
...
\end{equation}
In general, however, the expression
\begin{equation}
	\mat{e}^{(n)}
	= \matFnfull \tp{\left(\mat{C}^{(n)}\right)} \mat{C}^{(n)} \mat{S}
	- \mat{S} \tp{\left(\mat{C}^{(n)}\right)} \mat{C}^{(n)} \matFnfull
	\label{eqn:PulayError}
\end{equation}
is not zero.
In many cases $\mat{C}^{(n+1)}$
is, however,
much closer to the minimum $\mat{C}$ than $\mat{C}^{(n)}$ was,
...
and we will discuss its properties
and common algorithms in more detail in the next chapter.
...
From the unfilled shell property $\varepsilon_{\Nelec} < \varepsilon_{\Nelec+1}$,
point 7. of theorem \ref{thm:MathematicalHF},
...
Together with point 5., we see that only eigenfunctions $\psi_i$
...
In this section we now discussed roughly three ways to solve the \HF
equations in a discrete setting of a finite basis set $\{\varphi_i\}_{i\in\Ibas}$.
Firstly as a minimisation on a Stiefel manifold \eqref{eqn:HFStiefel},
where the energy is optimised with respect to the occupied orbital coefficients
$\mat{C}$.
The second option is to minimise on a Grassmann manifold \eqref{eqn:HFGrassmann}
and optimise the energy with respect to the density matrix $\mat{D}$.
The last option is to follow a self-consistent field approach
and in this manor construct a sequence $\mat{C}^{(n)}$
of orbital coefficients or of density matrices $\mat{D}^{(n)}$
until \eqref{eqn:HFDiscreteEquations} is satisfied,
\ie until self-consistency has been reached.
In section \vref{sec:SCFAlgorithms} we will discuss algorithms
for all these approaches.

\subsection{Restricted Hartree-Fock}
We already stated in equation \eqref{eqn:UHFS2}
...
\todoil{Maybe expand this paragraph}
Enforcing the correct $S^2$ value \eqref{eqn:UHFS2}
for open-shell electronic systems,
where (by convention) $\NelecA \geq \NelecB$,
is considerably more involved.
The first approach was published by \citeauthor{Roothaan1960}
~\cite{Roothaan1960,Alder1963}
and replaces the block-diagonal Fock matrix in \eqref{eqn:HFDiscreteEquations}
by a specifically crafted block-wise linear combination
of $\mat{F}^\alpha$ and $\mat{F}^\beta$,
which ensures that the resulting SCF minimum is an eigenfunction of $S^2$.
We do not consider this approach further in this thesis.
Instead we treat all open-shell systems with the \UHF procedure.
Firstly because \UHF is a much simpler algorithm to implement
and secondly because it is more widespread.
Nevertheless some issues,
which were found in our convergence analysis of a
Coulomb-Sturmian based Hartree-Fock ansatz~(see chapter \vref{ch:CSQChem})
could be appointed to using \UHF.
We will discuss this further in section \vref{sec:IssuesUHF}.

\subsection{Real-valued Hartree-Fock}
In our discussion of Hartree-Fock up to this point
the resulting numerical problems take place in complex arithmetic,
since both the coefficient matrix $\mat{C}$
as well as the Fock matrix $\mat{F}$ are complex-valued.
Doing this is possible, but is typically avoided by reducing
the \HF problem to an problem of equivalent structure,
...
The main motivation for this is that numeric operations
amongst complex numbers are slower since effectively
more floating point operations need to be performed
in order to treat both the real as well as the imaginary part
as required.


\noindent
Let $(\varepsilon_i, \psi_i) \in \R \times H^2(\R^3, \C^2)$ be an eigenvalue
...
we need need the real and the complex part of the right hand side
...
its real part $\psi_i^R$ and its imaginary part $\psi_i^I$ both must be
...
This implies that one can obtain the solution for the complex-valued problem
...
we defined previously in this section will only consist of real elements.
...
restore the full complex eigenspace one would obtain
from solving the full-fletched complex-valued problem
by linear combinations with complex coefficients.
The eigenstates we obtain from an entirely real-valued ansatz
are thus still the physical eigenstates we are after.
