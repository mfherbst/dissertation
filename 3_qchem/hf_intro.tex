...
\begin{rem}[Discretised \HF variational problem]
	...
	\begin{equation}
		...
		\label{eqn:HFStiefel}
	\end{equation}
	One can show~\cite{Cances2000} that the minimiser $\mat{C}_0$ is unique
...
This generalised eigenvalue problem can be solved for up $\Norb \leq \Nbas$ eigenpairs
$(\varepsilon_i, \vec{c}_i)$
using one of the algorithms described in section \vref{sec:DiagAlgos}
with the modifications for generalised eigenvalue problems discussed in
...
the minimiser $\mat{C} \in \mathcal{C}$ of \eqref{eqn:HFStiefel}
satisfies exactly the discretised \HF equations \eqref{eqn:HFDiscreteEquations}
such that
\[
	\matFfull \mat{C} = \mat{S} \mat{C} \mat{E}
\]
where
\[
	\mat{E}^{(n)} = \text{diag}(\varepsilon_1^{(n)}, \varepsilon_2^{(n)}, \ldots,
		\varepsilon_{\Nelec}^{(n)}).
\]
This condition can be equivalently expressed as~\cite{Pulay1982}
\begin{equation}
	\matFfull \mat{C}\tp{\mat{C}} \,
	\mat{S} - \mat{S} \, \mat{C}\tp{\mat{C}} \matFfull
	= \mat{0}
...
\end{equation}
Doing so we can in general not expect the expression
\begin{equation}
	\mat{e}^{(n)}
...
\end{equation}
to be zero.
In many cases, however, $\mat{C}^{(n+1)}$ is
...
In this section we now discussed roughly three ways to view the \HF
problem in a discrete setting of a finite basis set $\{\varphi_i\}_{i\in\Ibas}$.
