...
\todoil{Check the following statement with someone who knows}
One can identify
...
We will often use the symbol $\Psi$ to refer to Slater determinant from
...
Returning to our question which domain to use for $\Op{H}_{\Nelec}$,
we typically would use the domain
\[ D(\Op{H}_{\Nelec}) = H^2(\R^{3\Nelec}, \C) \cap \bigwedge^{\Nelec} L^2(\R^3, \C). \]
for $\Op{H}_{\Nelec}$ to be self-adjoint
and comply to condition \eqref{eqn:AsymElecWavefunction} as well.
For completeness let us not that the form domain in this case would be
...
With the HVZ theorem and Zhislin 61


Result:
\begin{itemize}
	\item Essential spectrum below continuous
	\item Ground state always exist
	\item Numerical modelling possible
\end{itemize}
...
In the general case of an arbitrary molecular system \emph{including} spin,
the determination of the spectral properties of $\Op{H}_{\Nelec}$
can be rather involved,
since we are dealing with the complex Hilbert space $L^2(\R^{3\Nelec}, \C^2)$
and a non-isotropic problem.
For our discussion we will therefore restrict ourselves
in this section to spin-free atomic systems.
As sketched in \cite{Teschl2014} and \cite{Reed1978}
the results we obtain here can be largely
generalised to molecular systems, however.
