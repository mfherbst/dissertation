...
linear algebra library as well as the ARPACK~\cite{Lehoucq1998} package
...
dense diagonalisation methods from LAPACK~\todo{cite}
...
\begin{figure}
	...
	{Code fragment of a simple basis-type independent
		...
		See section \ref{sec:LazytenExamples} for a detailed
...
and the desired discretisation and returns with
the lazy matrices \texttt{T}, \texttt{V}, \texttt{J}, \texttt{K}
and \texttt{S},
...
the type of basis function used to discretise the problem.
The reason for this is that \lazyten automatically adapts the
...
\texttt{T}, \texttt{V}, \texttt{J}, \texttt{K} and \texttt{S}
and usually%
...
for a \CS-based discretisation and use dense eigensolves for a \cGTO-based
discretisation.
This implies that \lazyten becomes a very effective
abstraction layer between the details of the lazy matrix implementation
and the \SCF algorithm.

Notice further that the \SCF of figure \ref{fig:LazytenRhfCode}
resembles the matrix expressions one would derive on paper
up to a very large extent
even though all involved matrices like
\texttt{T}, \texttt{V}, \texttt{J}, \texttt{K} and \texttt{S}
could be lazy.
Adding an extra term to the Fock matrix in line 17 would be very easy
and irrespective of the term being represented by a lazy
or a stored matrix,
it would be taken into account appropriately during the diagonalisation.
...
employed lazy matrices the code can be fixed but still flexible
for teaching and experimentation.
Even though the emphasis is on \contraction-based code in \lazyten
it can be used equivalently for dense matrices as well.
